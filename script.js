$(document).ready(() => {


// var apiKey = "eab6d01fa24f92ffa99be7e88ac10b4b"
// queryURL = ("api.openweathermap.org/data/2.5/weather?q=" + city + "&appid=" + apiKey);
// fiveDayqueryURL = ("api.openweathermap.org/data/2.5/forecast?q=" + city + "&appid=" + apiKey);

$("#submitBtn").on("click", function(){
    event.preventDefault();
    var city = $("#city").val();
    console.log(city);


    var apiKey = "eab6d01fa24f92ffa99be7e88ac10b4b";
    var queryURL = ("api.openweathermap.org/data/2.5/weather?q=" + city + "&appid=" + apiKey);
    var fiveDayqueryURL = ("api.openweathermap.org/data/2.5/forecast?q=" + city + "&appid=" + apiKey);

    $.ajax({url: queryURL, method: "GET"}).then(function(response){
        console.log(response);
    })


})
//ajax url and get
function cityResultBlock(){
    
}
//get the city val on click and combine qureryURL/API
//view file to get parameters
//get value and append to search block by creating a new element first
//store paraments and append to page


})//ready closing bracket



// **API RESPONE SHOULD SHOW 
// {"coord": { "lon": 139,"lat": 35},
//   "weather": [
//     {
//       "id": 800,
//       "main": "Clear",
//       "description": "clear sky",
//       "icon": "01n"
//     }
//   ],
//   "base": "stations",
//   "main": {
//     "temp": 281.52,
//     "feels_like": 278.99,
//     "temp_min": 280.15,
//     "temp_max": 283.71,
//     "pressure": 1016,
//     "humidity": 93
//   },
//   "wind": {
//     "speed": 0.47,
//     "deg": 107.538
//   },
//   "clouds": {
//     "all": 2
//   },
//   "dt": 1560350192,
//   "sys": {
//     "type": 3,
//     "id": 2019346,
//     "message": 0.0065,
//     "country": "JP",
//     "sunrise": 1560281377,
//     "sunset": 1560333478
//   },
//   "timezone": 32400,
//   "id": 1851632,
//   "name": "Shuzenji",
//   "cod": 200
// }